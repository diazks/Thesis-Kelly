...
data <- data %>% filter(year >= 2001, year <= 2018) #only >10 increments/year (should be >30/50 increments/year)
...
```{r}
source("D:/OneDrive - UGent/FWO/@FWO/01_fwo_phd/fwo_phd_git/HighstatLibV12.R") # support
...
# should take the 6th model.f because the 5-best models are singular
M4.f <- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 | FishID) + (1 + c.log.age| fyear) + c.sbt*c.f, data, REML = F)
...
M4.ssb <- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 | FishID) + (1 + c.log.age| fyear) + c.ssb, data, REML = F)
...
# best model: M4.f 
# Refit with REML 
M4.reml <- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 | FishID) + (1 + c.log.age| fyear) + c.sbt*c.f, data, REML = T)
```

```{r}
# try model with age >= 2 (no singular (1 + c.log.age|FishID))
data2 <- data %>% filter(age >= 2)
M4.f.age2 <- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 + c.log.age| FishID) + (1 + c.log.age| fyear) + c.sbt*c.log.age + c.f*c.log.age + c.sbt*c.f, data2, REML = F, na.action = "na.fail")
