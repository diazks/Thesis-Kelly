...
ggplot(data = data_sum_yearclass, aes(x = Cohort, y = AgeAtCapture, size = NumberFishSampled)) + geom_point(alpha = 0.5) + 
  facet_wrap(~ IcesAreaGroup) + 
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank()) +
...
Modnames <- c('M1b', 'M2a', 'M2b') #no singular fit
aictab(cand.set = models, modnames = Modnames, sort = TRUE)
...
# Model comparison - Fishing Pressure and Spawning Stock Biomass
...
#### sbt.within vs sbt.among
```{r}
M4.test <- lmer(log(increment) ~ c.log.age + c.log.aac + method + (1 | FishID) + (1 + c.log.age| fyear) + sbt.within*c.log.age + sbt.among*c.log.age + c.f*c.log.age + sbt.within*c.f + sbt.among*c.f, data, REML = F, na.action = "na.fail")
model.compare.extrinsic.sbt <- dredge(M4.test, fixed = c("c.log.age", "method", "c.log.aac", "sbt.within", "sbt.among"))
model.compare.extrinsic.sbt

model.compare.extrinsic.sbt$AICc[1]
model.compare.extrinsic.ssb$AICc[1]
model.compare.extrinsic.f$AICc[1]

# best extrinsic model
M4.reml <- lmer(log(increment) ~ c.log.age + c.log.aac + method + (1 | FishID) + (1 + c.log.age| fyear) + sbt.within*c.log.age + sbt.among, data, REML = T)
...
##### Temperature among and within individual
...
# Nnew variables for the mean temperature experienced by an individual across its lifetime (sbt.among) and the deviation of each observation from the individual-specific mean (sbt.within)
data$sbt.among<-ave(data$sbt,data$FishID)
data<-ddply(data, .(FishID), transform, sbt.within = scale(sbt,scale=F))

# M5 within and among individual temperature - just fixed effects
M5<- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 | FishID) + (1 + c.log.age| fyear) + c.sbt*c.log.age + c.f*c.log.age + c.sbt*c.f + sbt.among + sbt.within, data, REML = F, na.action = "na.fail")
# No singular fit
model.compare.among.stb <- dredge(M5, fixed = c("c.log.age", "method", "c.log.aac"))
model.compare.among.stb
# best model: c.f + c.log.aac + c.log.age + mth + c.f:c.log.age
M5<- lmer(log(increment) ~ c.log.age + method + c.log.aac + (1 | FishID) + (1 + c.log.age| fyear) + c.sbt*c.log.age + c.f*c.log.age + c.sbt*c.f + sbt.among + sbt.within, data, REML = F)

# M4.f (after dredge)
M4.f <- lmer(log(increment) ~ c.log.age + method + c.log.aac + c.f + (1 | FishID) + (1 + c.log.age| fyear) + c.log.age*c.f, data, REML = F)


## Comparison of models - among/within and M4.f (fishing)
models<-list(M5,M4.f)
Modnames <- c('M5','M4.f')
aictab(cand.set = models, modnames = Modnames, sort = TRUE)
# M4.f best AICc model





# Individual-specific differences (fitting random sbt.within slope for each FishID)
# M6 within and among individual temperature - with random within slope
# M6 <- lmer(log(increment) ~ c.log.age + method + c.log.aac + c.f + (1 | FishID) + (1 + c.log.age| fyear) + c.log.age*c.f + sbt.among + sbt.within + (sbt.within |FishID), data, REML = T, na.action = "na.fail")
...
# M6 best AICc
# Evidence of individual-specific reaction norms. Majority of the population-level temperature effect due to within-individual differences in lifetime temperature?




sbt.within.plot <- as.data.frame (effect(c('sbt.within'), M4.reml, xlevels = 19))
...
```{r}
sbt.among.plot <- as.data.frame (Effect (c('amongIDV'), M4.reml, xlevels = 19))
