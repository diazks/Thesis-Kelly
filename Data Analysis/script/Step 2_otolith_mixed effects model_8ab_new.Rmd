...
M5<- lmer(log(increment) ~ c.log.age + method + c.log.aac + c.f + (1 | FishID) + (1 + c.log.age| fyear) + c.log.age*c.f + sbt.among + sbt.within, data, REML = F, na.action = "na.fail")
# No singular fit
model.compare.among.stb <- dredge(M5, fixed = c("c.log.age", "method", "c.log.aac"))
model.compare.among.stb
# not among not within in best model
M5<- lmer(log(increment) ~ c.log.age + method + c.log.aac + c.f + (1 | FishID) + (1 + c.log.age| fyear) + c.log.age*c.f + sbt.among + sbt.within, data, REML = F)
